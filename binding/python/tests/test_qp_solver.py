...
class TestFrictionCone(unittest.TestCase):
    def test_cone1(self):
        ...
        for v in cone.generators:
            ...
            self.assertAlmostEqual(
                v.x() ** 2 + v.y() ** 2, v.z() ** 2 * mu**2, delta=0.00001
            )
    ...
    def test_cone2(self):
        ...
        for v in cone.generators:
            ...
            self.assertAlmostEqual(
                v.z() ** 2 + v.y() ** 2, v.x() ** 2 * mu**2, delta=0.00001
            )
...
class TestQPTask(unittest.TestCase):
    ...
    def test_posture_task(self):
        postureTask = tasks.qp.PostureTask(
            self.mbs, 0, [[], [0.2], [0.4], [-0.8]], 10, 1
        )
    ...
    def test_com_task(self):
        comD = eigen.Vector3d(
            sva.RotZ(math.pi / 4) * rbdyn.computeCoM(self.mbs[0], self.mbcInit)
        )
    ...
    def test_lin_velocity_task(self):
        linVelocityTask = tasks.qp.LinVelocityTask(
            self.mbs, 0, "b3", -eigen.Vector3d.UnitX() * 0.005
        )
        linVelocityTaskSp = tasks.qp.SetPointTask(
            self.mbs, 0, linVelocityTask, 1000, 1000
        )
...
class TestQPConstr(unittest.TestCase):
    def setUp(self):
        ...
        self.oriTask = tasks.qp.OrientationTask(
            self.mbs, 0, "b3", sva.RotZ(math.pi / 2)
        )
    ...
    def test_motion_constr(self):
        ...
        motionCstr = tasks.qp.MotionConstr(
            self.mbs, 0, tasks.TorqueBound(torqueMin, torqueMax)
        )
        ...
        self.assertAlmostEqual(
            (
                rbdyn.dofToVector(self.mbs[0], self.mbcs[0].jointTorque)
                - rbdyn.dofToVector(self.mbs[0], mbcTest.jointTorque)
            ).norm(),
            0,
            delta=0.00001,
        )
...
class TestQPJointLimits(unittest.TestCase):
    def setUp(self):
        ...
        self.posTask = tasks.qp.PositionTask(
            self.mbs,
            0,
            "b3",
            sva.RotZ(math.pi / 2) * self.mbcInit.bodyPosW[self.bodyI].translation(),
        )
        self.posTaskSp = tasks.qp.SetPointTask(self.mbs, 0, self.posTask, 10, 1)
    ...
    def test_joint_limits(self):
        ...
        jointConstr = tasks.qp.JointLimitsConstr(
            self.mbs, 0, tasks.QBound(lBound, uBound), 0.001
        )
    ...
    def test_damper_joint_limits(self):
        ...
        dampJointConstr = tasks.qp.DamperJointLimitsConstr(
            self.mbs,
            0,
            tasks.QBound(lBound, uBound),
            tasks.AlphaBound(lVel, uVel),
            0.125,
            0.025,
            1.0,
            0.001,
...
class TestQPTorqueLimits(unittest.TestCase):
    ...
    def test_motion_constr(self):
        ...
        uBound = [[], [30], [30], [30]]
        motionCstr = tasks.qp.MotionConstr(
            self.mbs, 0, tasks.TorqueBound(lBound, uBound)
        )
        ...
        for i in range(self.nrIter):
            ...
            for i in range(3):
                self.assertGreater(
                    self.mbcs[0].jointTorque[i + 1][0], lBound[i + 1][0] - 0.001
                )
                self.assertLess(
                    self.mbcs[0].jointTorque[i + 1][0], uBound[i + 1][0] + 0.001
                )
    ...
    def test_motion_poly_constr(self):
        ...
        motionPolyCstr = tasks.qp.MotionPolyConstr(
            self.mbs, 0, tasks.PolyTorqueBound(lBoundPoly, uBoundPoly)
        )
        ...
        for i in range(self.nrIter):
            ...
            rbdyn.forwardKinematics(self.mbs[0], self.mbcs[0])
            rbdyn.forwardVelocity(self.mbs[0], self.mbcs[0])
            motionPolyCstr.computeTorque(
                self.solver.alphaDVec(), self.solver.lambdaVec()
            )
            motionPolyCstr.torque(self.mbs, self.mbcs)
            for i in range(3):
                self.assertGreater(
                    self.mbcs[0].jointTorque[i + 1][0],
                    eigen.poly_eval(lBoundPoly[i + 1][0], oldQ[i + 1][0]),
                )
                self.assertLess(
                    self.mbcs[0].jointTorque[i + 1][0],
                    eigen.poly_eval(uBoundPoly[i + 1][0], oldQ[i + 1][0]),
                )
...
class TestQPAutoColl(unittest.TestCase):
    def test(self):
        ...
        posTask = tasks.qp.PositionTask(
            mbs, 0, "b3", mbcInit.bodyPosW[bodyI].translation()
        )
...
class TestQPBilatContact(unittest.TestCase):
    def test(self):
        ...
        motionCstr = tasks.qp.MotionConstr(
            mbs, 0, tasks.TorqueBound(torqueMin, torqueMax)
        )
        plCstr = tasks.qp.PositiveLambda()
        ...
        bi = [
            tasks.qp.BilateralContact(
                0, 1, "b0", "b0", points, biFrames, sva.PTransformd.Identity(), 3, 0.7
            )
        ]
...
def compute6dErrorInB1(b1, b2):
    assert isinstance(b1, sva.PTransformd) and isinstance(b2, sva.PTransformd)
    error = sva.MotionVecd(
        sva.rotationError(b1.rotation(), b2.rotation()),
        b1.translation() - b2.translation(),
    )
...
def computeDofError(b1, b2, dof):
    assert (
        isinstance(b1, sva.PTransformd)
        and isinstance(b2, sva.PTransformd)
        and isinstance(dof, eigen.MatrixXd)
    )
...
class TestQPDofContacts(unittest.TestCase):
    def test(self):
        ...
        self.assertAlmostEqual(
            computeDofErrorInB1(
                X_b1_cf * mbcs[0].bodyPosW[0],
                X_b1_cf * mbcInit.bodyPosW[0],
                contactDof,
            ),
            0,
            delta=1e-6,
        ...
        self.assertGreater(
            compute6dErrorInB1(
                X_b1_cf * mbcs[0].bodyPosW[0], X_b1_cf * mbcInit.bodyPosW[0]
            )[5]
            ** 2,
            0.1,
        )
...
class TestQPBoundedSpeed(unittest.TestCase):
    def test(self):
        ...
        posTask = tasks.qp.PositionTask(
            mbs, 0, bodyName, eigen.Vector3d(1, -1, 1), bodyPoint.translation()
        )
...
class TestSurfaceOrientationTask(unittest.TestCase):
    def test(self):
        ...
        surfOriTask = tasks.qp.SurfaceOrientationTask(
            mbs,
            0,
            "b3",
            sva.RotZ(math.pi / 2),
            sva.PTransformd(eigen.Vector3d(0, 0, 0)),
        )
...
class TestJointsSelector(unittest.TestCase):
    def test(self):
        ...
        for i in range(2):
            self.assertEqual(
                js1.selectedJoints()[i].posInDof, js2.selectedJoints()[i].posInDof
            )
            self.assertEqual(js1.selectedJoints()[i].dof, js2.selectedJoints()[i].dof)
        # Check joints are sorted
        self.assertLess(
            js1.selectedJoints()[0].posInDof, js1.selectedJoints()[1].posInDof
        )
...
class TestQPTransformTask(unittest.TestCase):
    def test(self):
        ...
        # Test TransformTask
        X_b_s = sva.PTransformd(
            eigen.Quaterniond(eigen.Vector4d.Random().normalized()),
            eigen.Vector3d.Random(),
        )
        X_0_t = sva.PTransformd(
            eigen.Quaterniond(eigen.Vector4d.Random().normalized()),
            eigen.Vector3d.Random(),
        )
