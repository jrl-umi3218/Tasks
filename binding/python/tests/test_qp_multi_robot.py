...
class TestTwoArmContact(unittest.TestCase):
    def test(self):
        ...
        if (
            platform.architecture()[0] == "32bit" or platform.machine() == "aarch64"
        ) and solver.solver() == b"QLD":
            oriD = sva.RotZ(0)
        ...
        for i in range(1000):
            ...
            X_b1_b2_post = X_0_b2_post * X_0_b1_post.inv()
            self.assertAlmostEqual(
                (X_b1_b2.matrix() - X_b1_b2_post.matrix()).norm(), 0, delta=1e-5
            )
...
class TestTwoArmDDynamicContact(unittest.TestCase):
    def test(self):
        ...
        contVecFail = [
            tasks.qp.UnilateralContact(
                0, 1, "b3", "b0", points, sva.RotX(-math.pi / 2), X_b1_b2, nrGen, 0.7
            )
        ]
...
class TestTwoArmMultiCoM(unittest.TestCase):
    def test(self):
        ...
        posture2Task = tasks.qp.PostureTask(mbs, 1, mbc2Init.q, 2, 1)
        comD = (
            rbdyn.computeCoM(mb1, mbc1Init) + rbdyn.computeCoM(mb2, mbc2Init)
        ) / 2 + eigen.Vector3d(0, 0, 0.5)
...
class TestMultiRobotTransform(unittest.TestCase):
    def test(self):
        mb1, mbc1Init = arms.makeZXZArm(
            True, sva.PTransformd(sva.RotZ(-math.pi / 4), eigen.Vector3d(-0.5, 0, 0))
        )
        rbdyn.forwardKinematics(mb1, mbc1Init)
        rbdyn.forwardVelocity(mb1, mbc1Init)

        mb2, mbc2Init = arms.makeZXZArm(
            False, sva.PTransformd(sva.RotZ(math.pi / 2), eigen.Vector3d(0.5, 0, 0))
        )
        rbdyn.forwardKinematics(mb2, mbc2Init)
        rbdyn.forwardVelocity(mb2, mbc2Init)
        ...
        mrtt = tasks.qp.MultiRobotTransformTask(
            mbs,
            0,
            1,
            "b3",
            "b3",
            sva.PTransformd(sva.RotZ(-math.pi / 8)),
